...
def funception(func1, start):
    ...
    """
    "*** YOUR CODE HERE ***"
    def func2(stop,start = start): # 记得要在括号里声明，否则局部变量识别不到
        if start < 0 :
            return None        
        elif start >= stop :
            return func1(start)
            ...
            total = 0
            while start < stop :
                total , start = product(start,func1), start + 1
            return total
    return func2
...
def mod_maker():
    ...
    True
    """
    return lambda x ,y: x % y or True # 这道题很有挑战性，想了很久
...
def add_results(f1, f2):
    ...
    44
    """
    return lambda x, f1 = f1, f2 = f2: f1(x)+f2(x)
