...
def is_always_roll(strategy, goal=GOAL):
    ...
    score1 = 0
    count = 0
    old_num = strategy(score0, score1)
    while (score0 < goal):
        while (score1 < goal):
            if (old_num != strategy(score1, score0)):
                count = 1
            score1 += 1
        score1 = 0
        if (old_num != strategy(score0,score1)):
            count = 1
        score0 += 1
    if (count == 1):
        return False
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    increase = square_update(0,score,opponent_score) - score ## 这个纠结了好久，还是没有理解square_update的函数原理
