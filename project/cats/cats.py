...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    fenzi = 0
    cnt1 = len(typed_words)
    cnt2 = len(source_words)
    ...
    for i in range(cnt1):
        for j in range(cnt2):
            if typed_words[i] == source_words[j] and i == j:
                fenzi += 1

    return (fenzi/cnt1) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    l = []
    for i in word_list:
        l.append(diff_function(typed_word, i, limit))
        if i == typed_word :
            return typed_word
